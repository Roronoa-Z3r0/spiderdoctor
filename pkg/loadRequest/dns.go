...
func ParseMetrics(final *DnsMetrics, validVals []float32) (*DnsMetrics, error) {
	...
	//		if len(v.answer) > 0 && v.rcode == dns.RcodeSuccess {
	//			final.SucceedCount++
	//			final.DnsAnswer = append(final.DnsAnswer, v.answer...)
	//			final.DnsAnswer = dns.Dedup(final.DnsAnswer, nil)
	//			validVals = append(validVals, float64(v.rtt))
	//		} else {
	//			final.FailedCount++
	//		}
	//		rcodeStr := dns.RcodeToString[v.rcode]
	...
	p := func(wg *sync.WaitGroup) {
		...
		if r.e != nil {
			...
			if len(r.msg.Answer) > 0 && r.msg.Rcode == dns.RcodeSuccess {
				final.SucceedCount++
				//final.DnsAnswer = append(final.DnsAnswer, r.msg.Answer...)
				//final.DnsAnswer = dns.Dedup(final.DnsAnswer, nil)
